...
def plots_with_outliers_removed(df):
    ...
    if choice==1:
        ...
        sb.pairplot(data=df, kind="scatter", diag_kind="hist", corner=True, height=1.5)
        plt.title("PairPlot")
        plt.show()

    elif choice==5:
        sb.heatmap(data=df[["Open", "High", "Low", "Close", "Adj Close", "Volume"]].corr(), annot=True, cmap="coolwarm")
        plt.title("HeatMap")
        plt.show()
